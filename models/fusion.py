"""
Multimodal Heterogeneous Federated Learning Fusion Network.

This module defines the central architecture for the federated framework,
combining a frozen ResNet-18 for satellite imagery, a TSMixer for rainfall 
time-series, and a NARX network for water-level sequences.
"""
...
class TriModalFloodNet(nn.Module):
    ...
    Modalities:
      - Satellite imagery  -> ResNet-18 (frozen) + Spatial Attention encoder
      - Rainfall time-series -> TSMixer encoder
      - Water-level NARX  -> NARX encoder

    The classifier receives the concatenated fusion vector after MC Dropout.
    When a modality is absent (img_only or sensor_only nodes), its branch 
    output is zeroed out so the same fusion classifier logic handles all 
    federated client types dynamically.

    Args:
        rate (float, optional): The model slice rate for HeteroFL (e.g., 0.5 
            for half the hidden dimensions, 1.0 for the full model). Defaults to 1.0.
    """

    # Branch output sizes for rate=1.0
    IMG_DIM = 64   # CNN output dimension
    ...
    def __init__(self, rate=1.0):
        ...
        self.mc_dropout1 = nn.Dropout(p=0.5)
